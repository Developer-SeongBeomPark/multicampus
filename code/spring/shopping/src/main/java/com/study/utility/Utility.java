...
public class Utility {
  ...
  public static String paging(int totalRecord, int nowPage, int recordPerPage, String col, String word) {
    ...
    for (int i = startPage; i <= endPage; i++) {
      ...
      if (nowPage == i) {
        ...
        str.append("<li><a class = 'page-link' href='./list?col=" + col + "&word=" + word + "&nowPage=" + i + "'>" + i + "</a></li>");
      }
    }
  ...
  public static String rpaging(int total, int nowPage, int recordPerPage, String col, String word, String url,
    ...
    for (int i = startPage; i <= endPage; i++) {
      ...
      if (nPage == i) {
        str.append("<li class='active'><a class = 'page-link' href=#>" + i + "</a></li>");
      } else {
    ...
    _nowPage = (nowGrp * pagePerBlock) + 1; // 10개 다음 페이지로 이동
    if (nowGrp < totalGrp) {
      str.append("<li><A class = 'page-link' href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + nowPage + "&nPage=" + _nowPage
          + "'>다음</A></li>");
    }
    str.append("</ul>");
    //str.append("</nav>");
